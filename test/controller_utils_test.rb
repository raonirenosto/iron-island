...
class ControllerUtilsTest < Minitest::Spec
  include ControllerUtils


  # def test_is_avaliable_command
  #   hash_symbols =
  #     {
  #       :get => ["pegar", "pega"],
  #       :go => ["ir", "quero ir"]
  #     }
  #
  #   assert is_avaliable_command? "pega tal coisa", :get, hash_symbols
  #   assert is_avaliable_command? "quero ir para tal lugar", :go, hash_symbols
  #   assert_equal false, is_avaliable_command?("cortar avore", :get, hash_symbols)
  #   assert_equal false, is_avaliable_command?("descer ladeira", :go, hash_symbols)
  #
  #   assert_raises RuntimeError do
  #     is_avaliable_command? "quero ir para tal lugar", :unavaliable_symbol, hash_symbols
  #   end
  # end
  #
  # def test_is_not_avaliable_command
  #   hash_symbols =
  #     {
  #       :get => ["pegar", "pega"],
  #       :go => ["ir", "quero ir"]
  #     }
  #   assert_equal true, is_not_avaliable_command?("descer ladeira", :go, hash_symbols)
  # end
  ...
  def test_get_symbol
    ...
    assert_equal :go, get_symbol("ir para tal lugar", hash_symbols)
    assert_nil nil, get_symbol("não sei para onde vou", hash_symbols)
    assert_equal :get, get_symbol("pegar tal coisa", hash_symbols)
  end

  def test_get_symbol_exclude
    hash_symbols =
      {
        :get => ["pegar", "pega"],
        :go => ["ir", "quero ir"],
        :iron_village => ["Vila de Ferro", "vila"]
      }
    symbols = get_all_symbols("ir para vila de ferro", hash_symbols)

    assert symbols.include? :go
    assert symbols.include? :iron_village
  end
